...
1. **Encoded commands**: Attackers routinely base64-encode their payloads (`-EncodedCommand` / `-enc`). Standard command-line logging only captures the encoded string -- Script Block Logging captures the **decoded** content after PowerShell processes it.
