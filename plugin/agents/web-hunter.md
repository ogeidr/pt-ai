...
Rules:
- **Required fields:** `schema_version` ("1.0"), `id` (`F-NNNN` — next unused; check the file's existing ids first), `title`, `target`, `category` (`web` for web findings; `network|ad|cloud|container|host|credential|cicd|mobile|other` otherwise), `severity` (`info|low|medium|high|critical`), `status`, `source_agent` (`web-hunter`), `discovered_at` (ISO-8601 UTC).
